package org.lance.other;

import org.lance.activity.R;
...
public class FileBrowser {
	...
	public FileBrowser(Activity activity) {
		...
		browseFilePanel = (LinearLayout) activity
				.findViewById(R.id.browseFilePanel);

		selected = false;
		selectedIndex = -1;
		selectedIcon = activity.getResources().getDrawable(
				R.drawable.question_red);
	...
	private void pack() {
		...
		for (int index = 0; index < length; index++) {
			fileView[index] = (LinearLayout) inflater.inflate(
					R.layout.adapter_file, null).findViewById(R.id.fileView);
	...
	private void addQueryEventListener(TextView fileFlag, final TextView text,
			final int index) {
		fileFlag.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				toastString = "Codec: " + codecName + "  Width: " + width
						+ "  Height: " + height;
	...
	private void addSelectEventListener(final TextView text, final int index,
			final LinearLayout[] fileView, final boolean isSelectedMediaFile) {
		text.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (selected == true) {
					fileView[selectedIndex].getChildAt(0).setBackgroundColor(
							falseColor);
					((TextView) fileView[selectedIndex].getChildAt(1))
				...
				if (isSelectedMediaFile == true) {
					fileView[index].getChildAt(0).setBackgroundDrawable(
							selectedIcon);
				}
